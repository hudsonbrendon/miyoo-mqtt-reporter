...
publish_discovery() {
    ...
    _emit "$did" sensor        mem_cached_kb "Miyoo RAM Cached"   "kB"   "{{ value_json.mem_cached_kb }}" "mdi:memory"          "data_size"
    _emit "$did" sensor        mem_buffers_kb "Miyoo RAM Buffers" "kB"   "{{ value_json.mem_buffers_kb }}" "mdi:memory"         "data_size"
    _emit "$did" sensor        disk_read_sectors "Miyoo Disk Read Sectors" "" "{{ value_json.disk_read_sectors }}" "mdi:harddisk-plus" ""
    _emit "$did" sensor        disk_write_sectors "Miyoo Disk Write Sectors" "" "{{ value_json.disk_write_sectors }}" "mdi:harddisk-minus" ""
    _emit "$did" sensor        apps_count  "Miyoo Apps Installed" ""     "{{ value_json.apps_count }}"  "mdi:apps-box"          ""
    _emit "$did" sensor        emulators_count "Miyoo Emulators"  ""     "{{ value_json.emulators_count }}" "mdi:nintendo-game-boy" ""
    _emit "$did" sensor        themes_count "Miyoo Themes"        ""     "{{ value_json.themes_count }}" "mdi:palette-swatch"   ""
    _emit "$did" sensor        saves_count "Miyoo Save Files"     ""     "{{ value_json.saves_count }}" "mdi:content-save-all"  ""
    _emit "$did" sensor        saves_size_kb "Miyoo Saves Size"   "kB"   "{{ value_json.saves_size_kb }}" "mdi:database"        "data_size"
    _emit "$did" sensor        session_duration_sec "Miyoo Session Duration" "s" "{{ value_json.session_duration_sec }}" "mdi:timer-play" "duration"
}
